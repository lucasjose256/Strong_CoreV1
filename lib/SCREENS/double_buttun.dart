...
/// {@endtemplate}
class AddTodoButtonDouble extends StatefulWidget {
  ...
  String numberId = '';
  AddTodoButtonDouble({this.number, this.newColor, required this.numberId});

  @override
  State<AddTodoButtonDouble> createState() => _AddTodoButtonDoubleState();
}

class _AddTodoButtonDoubleState extends State<AddTodoButtonDouble> {
  @override
...
class AddTodoPopupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        Expanded(child: CardColor(context, ' numID')),
        Expanded(child: CardColor(context, ' nID')),
        /* CardColor(context, 'O'),
        CardColor(context, 'X'),*/
      ],
    );
  ...
  Hero CardColor(BuildContext context, String numID) {
    return Hero(
      tag: _heroAddTodo + '$nome' + numID,
      createRectTween: (begin, end) {
        return CustomRectTween(begin: begin!, end: end!);
      },
      child: Material(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('Selecione o grau da dor no Ombro Direito'),
          const Divider(
            color: Colors.white,
            thickness: 0.2,
          ),
